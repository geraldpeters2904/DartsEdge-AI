...
class ForwardScheduleMonitor:
    ...
    def _copy(self, **changes):
        ...
        self._status = ForwardScheduleMonitorStatus(**values)
        return self._status
    ...
    def start(self):
        with self._lock:
            ...
            self._thread = threading.Thread(
                target=self._run,
                name="dartsedge-forward-schedule-monitor",
                daemon=True,
            )
            self._thread.start()
            return self._status
    ...
    def stop(self):
        ...
        with self._lock:
            return self._copy(
                running=False,
                next_run_at=None,
                last_message="Forward schedule monitor stopped.",
            )
    ...
    def _run(self):
        ...
        with self._lock:
            self._copy(running=False, next_run_at=None)
