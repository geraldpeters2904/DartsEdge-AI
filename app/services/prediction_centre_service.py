...
def build_prediction_centre(db, *, limit: int = 30) -> Dict[str, Any]:
    ...
    for card in cards:
        ...
        card["sort_ev"] = float(assessment.expected_value_percent) if assessment else -999.0
        card["sort_edge"] = float(assessment.edge_percent) if assessment else -999.0
        card["sort_stake"] = float(assessment.recommended_stake) if assessment else 0.0
        card["competition"] = card["fixture"].tournament or "Tournament not set"
