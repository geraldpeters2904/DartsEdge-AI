...
_TABLE_CANDIDATES = {
    ...
    "fixtures": ("fixtures", "canonical_fixtures", "match_fixtures"),
...
class WarehouseDashboardService:
        ...
        *,
        ...
        metrics = [
            WarehouseMetric(
                key="players",
                label="Players",
                value=table_counts["players"],
                detail="Tracked warehouse players",
            ),
            WarehouseMetric(
                key="fixtures",
                label="Fixtures",
                value=table_counts["fixtures"],
                detail="All canonical fixtures",
            ),
            WarehouseMetric(
                key="matches",
                label="Matches",
                value=table_counts["matches"],
                detail="Imported match results",
            ),
            WarehouseMetric(
                key="statistics",
                label="Statistics",
                value=table_counts["statistics"],
                detail="Player-match statistic rows",
            ),
            WarehouseMetric(
                key="scheduled",
                label="Scheduled",
            ...
            WarehouseMetric(
                ...
                value=capture_library.summary.captured_matches,
                detail=(
                    f"of {capture_library.summary.expected_matches} "
                    "expected capture pages"
                ),
            ),
            WarehouseMetric(
                key="missing",
                label="Missing Pages",
                value=capture_library.summary.missing_matches,
                detail="Outstanding browser-assisted captures",
            ),
        ]
    ...
    def _first_existing(
        ...
        return next(
            (name for name in candidates if name in table_names),
            None,
        )
    ...
    def _count_status(
        ...
        status_column = next(
            (
                name for name in
                ("status", "match_status", "fixture_status")
                if name in columns
            ),
            None,
        )
        ...
        placeholders = ", ".join(
            f":status_{index}" for index in range(len(statuses))
        )
        params = {
    ...
    def _build_health(
        *,
        ...
        if resolved["fixtures"]:
            items.append(
                WarehouseHealthItem(
                    ...
                    status="healthy",
                    detail=f'Using table "{resolved["fixtures"]}".',
                )
            )
        else:
            items.append(
                WarehouseHealthItem(
                    label="Fixture warehouse",
                    status="warning",
                    detail="No recognised fixture table exists yet.",
                )
            )
        ...
        if resolved["players"]:
            items.append(
                WarehouseHealthItem(
                    label="Player warehouse",
                    status="healthy",
                    detail=f'Using table "{resolved["players"]}".',
                )
            )
        else:
            items.append(
                WarehouseHealthItem(
                    label="Player warehouse",
                    status="warning",
                    detail="No recognised player table exists yet.",
                )
            )
        ...
        items.append(
            WarehouseHealthItem(
                ...
                status="healthy" if missing == 0 else "warning",
                detail=(
        ...
        items.append(
            WarehouseHealthItem(
                ...
                status="healthy",
                detail=f"{len(table_names)} table(s) detected.",
            )
        )
        return items
    ...
    def _recent_imports(
        ...
        id_column = next(
            (
                name for name in
                ("preview_uuid", "import_uuid", "id", "uuid")
                if name in columns
            ),
            None,
        )
        created_column = next(
            (
                name for name in
                ("created_at", "started_at", "imported_at", "updated_at")
                if name in columns
            ),
            None,
        )
        ...
        return [
            WarehouseRecentImport(
                ...
                created_at=(
                    str(row.get(created_column))
                    if created_column and row.get(created_column) is not None
                    else None
                ),
                status=str(
                    row.get(status_column, "unknown")
                    if status_column else "unknown"
                ),
                provider=str(
                    row.get(provider_column, "—")
                    if provider_column else "—"
                ),
                competition=str(
                    row.get(competition_column, "—")
                    if competition_column else "—"
                ),
            )
            for row in rows
        ]
