...
class DecisionEngineTests(unittest.TestCase):
    ...
    def test_paper_mode_never_returns_live_accept(self):
        ...
        rules = strategy_rules(paper)
        rules["enforcement_mode"] = "active"
        replacement = update_strategy(
            self.db,
            paper.id,
            name=paper.name,
    ...
    def test_strong_opportunity_receives_high_score(self):
        result = self._decision(
            model_probability=76,
            confidence_percent=88,
            expected_value_percent=22,
            edge_percent=16,
            official_stake=20,
            sample_size=100,
            portfolio_exposure_percent=2,
        )
        self.assertGreaterEqual(result.decision_score, 80)
        self.assertIn(result.decision_grade, {"Strong", "Elite"})
        self.assertTrue(result.positive_reasons)

    def test_blocked_opportunity_is_capped_as_pass(self):
        result = self._decision(
            model_probability=52,
            confidence_percent=45,
            expected_value_percent=-5,
            edge_percent=-2,
            official_stake=0,
            sample_size=0,
        )
        self.assertFalse(result.qualifies)
        self.assertLessEqual(result.decision_score, 59)
        self.assertEqual(result.decision_grade, "Pass")
        self.assertTrue(result.negative_reasons)

    def test_summary_contains_v2_fields(self):
        result = self._decision()
        payload = summary(result)
        self.assertEqual(payload["decision_score"], result.decision_score)
        self.assertEqual(payload["decision_grade"], result.decision_grade)
        self.assertIsInstance(payload["positive_reasons"], list)
        self.assertIsInstance(payload["negative_reasons"], list)
