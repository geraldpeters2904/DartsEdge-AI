...
class StrategyFrameworkTests(unittest.TestCase):
    ...
    def test_strategy_api_route(self):
        ...
        active_strategies = [item for item in payload["strategies"] if item["active"]]
        self.assertEqual(len(active_strategies), 1)
        self.assertEqual(payload["active"]["id"], active_strategies[0]["id"])
        self.assertIn(payload["active"]["name"], {item["name"] for item in payload["strategies"]})
